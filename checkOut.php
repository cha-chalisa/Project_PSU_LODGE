...
<?php //การแสดงห้องพักที่ไม่สามารถจองได้(ไม่ว่าง) เพื่อเข้าไปทำการ check out น.ส.ปวันรัตน์ สร้างเมือง 6130613051
    $connect = mysqli_connect("localhost","root","","class");

    $sql='SELECT RoomID FROM room WHERE RoomStatus="unavailable"';
    ...
    echo '<td>checkOut</td>';
		while ($row = mysqli_fetch_array($result)){
			echo '<form name="frmcheckOut'.$row['RoomID'].'" method="post" action="checkOut2.php">'."\n";
			echo '<tr>';
			...
			echo '<td><input class="btn-update" name="checkout" type="submit" value="checkout"></td>'."\n";
			echo '</tr>'."\n";
			echo '</form>';
		}
		echo '</table>';
